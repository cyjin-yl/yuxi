...
// workers/party-index.ts
var PartyIndexDO = class {
  constructor(state, _env) {
    this.state = state;
  }
  state;
  static {
    __name(this, "PartyIndexDO");
  }
  tableReady = false;
  ensureTable() {
    if (this.tableReady) return;
    this.state.storage.sql.exec(
      "CREATE TABLE IF NOT EXISTS rooms (code TEXT PRIMARY KEY, name TEXT NOT NULL, hostId TEXT NOT NULL, members INTEGER NOT NULL, updatedAt INTEGER NOT NULL)"
    );
    this.tableReady = true;
  }
  async fetch(request) {
    ...
    if (request.method === "POST" && action === "upsert") {
      ...
      if (!code) return json2({ error: "missing_code" }, 400);
    ...
    if (action === "list") {
      ...
      const rows = this.state.storage.sql.exec("SELECT code, name, hostId, members FROM rooms WHERE members > 0 ORDER BY members DESC, updatedAt DESC").toArray();
    ...
    return json2({ error: "unknown_action" }, 400);
  }
};
